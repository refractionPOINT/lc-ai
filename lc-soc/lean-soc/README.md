...
- **Fewer moving parts.** Four agents instead of eight means fewer API keys, fewer D&R rules, fewer things that can break. The coordination surface is minimal -- triage creates tickets, investigator processes them, responder acts on confirmed threats.
...
- **Reporter pulls double duty.** The daily reporter also handles SLA monitoring and stale ticket detection, combining the tiered model's SOC Manager and Shift Reporter into one agent.
...
| [investigator](investigator/) | Full investigation from triage to conclusion | opus | $5.00 | 15m | Ticket created |
| [responder](responder/) | Isolate sensors, block IOCs for confirmed threats | sonnet | $1.00 | 5m | Tag: needs-containment |
| [reporter](reporter/) | Daily SOC metrics, SLA monitoring, stale ticket cleanup | sonnet | $1.00 | 5m | Schedule: every 24h |

## Installation Order

1. **triage** -- starts creating tickets from detections
2. **investigator** -- starts investigating tickets as they're created
...
- Higher per-investigation cost ($5.00 vs $2.00 for L1-only) since every ticket gets the full treatment
