...
- **Directory audit events are NOT in the `entraid` adapter's feed.** Detections on Entra directory changes — app consent (`Consent to application`), OAuth2 permission grants (`Add OAuth2PermissionGrant`), app role assignments, user/group/role changes — live in the **AuditLogs** category of the diagnostic stream (`azure_event_hub` + `azure_ad`) or in the M365 UAL (`office365` adapter, `Audit.AzureActiveDirectory`, operation names carry a trailing period there). The `entraid` adapter polls only riskDetections and will NEVER deliver these events; do not select it just because the product is "Entra". Note that in the `azure_ad` stream the LC event type is the *category* (e.g. `AuditLogs`), so detection rules match on `event/operationName`.
- **`azure_monitor` is not a substitute label for `azure_ad`.** The Entra diagnostic stream uses the Azure Monitor envelope, so `azure_monitor` happens to parse it identically — but the sensor is then tagged as the Azure Monitor platform, breaking `is platform`/LCQL targeting and shared cross-tenant rules written against `azure_ad`. Use `azure_ad` for Entra streams; reserve `azure_monitor` for general Azure activity/resource logs.
