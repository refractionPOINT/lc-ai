...
| Native harness streams and usage | [adapters/](src/lc_eval/adapters/) |
...
Manifests record scenario revision/hash, seed, docs and fixture digests, evaluator and tool digests, worker image, harness/model/effort, permission profile, limits, repetition and LC location. Reports recover legacy location only from consistent persisted organization evidence, never the current configuration.
...
Claude Code and Codex are the live harnesses. Workspace is an explicit unsupported adapter. Optional API request-budget modules exist but are not connected to the live subscription controller. Native EDR enrollment and Cloud Security fixtures have not been proved by the initial three scenarios.
